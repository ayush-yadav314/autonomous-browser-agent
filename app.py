    ...
    try:
        if parsed["action"] == "TYPE":
            ...
            if el:
                ...
                return f"Typed '{parsed['text']}' into element [{parsed['element']}]"
            return f"Element [{parsed['element']}] not found"

        elif parsed["action"] == "CLICK":
            el = element_map.get(parsed["element"])
            if el:
                el.click(timeout=5000)
                return f"Clicked element [{parsed['element']}]"
            return f"Element [{parsed['element']}] not found"

        elif parsed["action"] == "DONE":
            return f"Agent finished. Final answer: {parsed['answer']}"

        return f"Could not understand action: {parsed.get('raw')}"

    except Exception as e:
        return f"Action failed (element may be blocked/hidden/gone): {type(e).__name__}"


def run_agent(start_url, goal, max_steps, log_area):
    logs = []
    ...
    with sync_playwright() as p:
        ...
        for step in range(max_steps):
            ...
            if decision == last_decision:
                log("Same action repeated twice — stopping to avoid infinite loop.")
                break
            ...
            result = execute_action(page, elements, parsed)
            log(result)
            ...
            if was_click and url_after_action != url_before_action:
                ...
                log("Goal achieved — action produced a page change.")
                break
...
st.set_page_config(page_title="Autonomous Browser Agent", layout="centered")
st.title("Autonomous Browser Agent")
st.caption("Local LLM (Ollama + Phi-3 Mini) + Playwright — fully free, runs on your machine")

start_url = st.text_input("Starting URL", value="https://duckduckgo.com")
goal = st.text_input("Goal", value="Search for 'AI agents in Python'")
max_steps = st.slider("Max steps", min_value=1, max_value=10, value=5)

if st.button("Run Agent"):
    st.write("Running... a browser window will open on your machine.")
    log_area = st.empty()
    run_agent(start_url, goal, max_steps, log_area)
